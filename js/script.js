  ...
  Swal.fire({
    ...
    if (result.dismiss === Swal.DismissReason.timer) {
      Swal.fire('Kalau belum jangan lupa makan yaa').then((result) => {
        Swal.fire({
          title: ' ',
          html: `Aku tau ko ayang lagi sibuk banget`,
          timer: 2000,
          timerProgressBar: true,
          showConfirmButton: false,
        }).then((result) => {
          if (result.dismiss === Swal.DismissReason.timer) {
            Swal.fire({
              ...
              if (result.dismiss === Swal.DismissReason.timer) {
                Swal.fire('Semangat terus ya ayang :)').then((result) => {
                  Swal.fire(
                    ...
                    Swal.fire(
                      ...
                      Swal.fire('Aku dukung ayang terus ko').then((result) => {
                        Swal.fire({
                          title: 'Oh iya aku mau nanya',
                          text: 'Ada sesuatu yang lagi ayang pingin ga?',
                          showDenyButton: true,
                          ...
                          if (result.isConfirmed) {
                            Swal.fire({
                              ...
                              inputValidator: (value) => {
                                if (!value) {
                                  return 'Isi dulu dong ayang'
                                }
                              }
                            }).then((result) => {
                              Swal.fire('Okey ayang', 'Nanti aku kabulin kemauan kamu yaa', 'success').then((result) => {
                                Swal.fire("See ya! Semangat ayang ^^").then((result) => {
                                  selesai()
                                });
                              })
                            })
